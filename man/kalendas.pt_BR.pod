...
cómputos estão desenvolvidos sobre três sistemas de datación: o calendário
Juliano, o calendário Gregoriano e o sistema de numeración de dia/data 
Juliana.
...
Os calendários Juliano e Gregoriano são sistemas de datación usados
pela Cultura Ocidental ou Cristã: o calendário Juliano, introduzido
pelo triunviro Julio César no 46 a.C., foi usado até o 1582 A.D.,
...
difieren no conteo dos anos bisiestos. O sistema de numeración de dias
...
Os cómputos têm por objetivo a conversão de datas entre os três
sistemas de datación mencionados anteriormente. A maioria de algoritmos
implementados usam a função modulo (o residuo da divisão entre dois
números inteiros) com o propósito de manejar variables inteiras ao longo
do cómputo, já que os calendários manejam números inteiros, com exceção
do sistema de numeración de dia/data Juliana que contém parte decimal
...
No mês com valor numérico: Ene-E<gt>1, Feb-E<gt>2, ..
...
A data Juliana. As partes inteira e decimal de 'C<fj>' representam o
número de dia Juliano e a fração de dia Juliano respectivamente.
Com cinco cifras decimales obtém-se a hora com uma precisão de 1
segundo. 'C<fj>' pode ser um número negativo.
...
Se não há algum parámetro na linha de comandos, se avalia a data Juliana
...
Em diversas aplicações astronómicas faz-se referência à época J2000,
...
programa, por rendimento incorrecto de argumentos.

=over 4

=item 'Muitos argumentos!'

A opção requerida tem mais de um argumento ou as comillas foram
omitidas. Correntes de carateres separados por espaços ou comas
(C<--fun arg1 arg2,arg3...>) são considerados como argumentos; por
tanto, a cada opção requer um argumento entre comillas. Ver Exemplos.

=item 'Entrada de data incorreta!'

Os argumentos ingressados por linha de comandos não se ajustam ao formato
requerido pela função. A cada argumento deve estar entre paréntesis. Para
...
de calendário 'C<fj>' é um número real positivo ou negativo. Ver Exemplos. 
...
C<kalendas>, versão 1.0.1, $Date: 2013/09/13 21:22:26 $

=head1 LICENÇA

Este programa é software livre: você pode redistribuirlo e/ou o modificar
...
de COMERCIABILIDAD ou APTIDÃO PARA UM PROPOSITO DETERMINADO. Para mais
